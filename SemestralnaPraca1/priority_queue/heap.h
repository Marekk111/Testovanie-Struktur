...
namespace structures
{
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		if (leftSonIndex >= PriorityQueueList<T>::size()) {
			return -1;
		}
		else {
			if (rightSonIndex >= PriorityQueueList<T>::size()) {
				return leftSonIndex;
			}
			else {
				return ((*PriorityQueueList<T>::list_)[leftSonIndex])->getPriority() <= ((*PriorityQueueList<T>::list_)[rightSonIndex])->getPriority() ? leftSonIndex : rightSonIndex;
			}
		}
	}
